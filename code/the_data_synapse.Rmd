...
## Read in some real data and repeat this!

``` {r, climate_data_input}
climate_df <- read.delim("../data/[IWMO+T]datatable.tsv", row.names = 1)
colnames(climate_df) <- sapply(colnames(climate_df), function(i){
  output <- gsub("X", "", i)
  output <- gsub("E.", "E", output)
  return(as.character(as.numeric(output)))
})
climate_df$month <- as.numeric(row.names(climate_df))

timepoints_from_2000 <- (climate_df$month > 40*12)
climate_df <- climate_df[timepoints_from_2000, ]
# NOTE FOR TIMINGS: 0 = January 1960, and months are relevant to this!

longitude_df <- read.delim("../data/longitude.txt", skip = 2, col.names = c("station", "longitude"))

station_names_df <- read.delim("../data/station_ids.txt", skip = 2, col.names = c("station", "name"))

# # impute the NAs with mean for now
